...
(function(w)
	{
		function getJsonI18N()
		{
			...
			if (binln = "null")
			{
				const lang = LANGUAGES.find(l => l.regex.test(navigator.language))
					.lang;
			}
			else
			{
	...
	{
		$(this)
			.attr('placeholder', I18N[this.dataset.placeholderI18n]);
	});
	...
	let fontunit = isDesktop ? 20 : ((window.innerWidth > window.innerHeight ? window.innerHeight : window.innerWidth) / 320) * 10; document.write('<style type="text/css">' +
	...
	function getCPS()
	{
		let cps = _gameScore / ((new Date()
			.getTime() - _gameStartDatetime) / 1000);
	...
	function shareText(cps)
	{
		if (mode === MODE_NORMAL)
		{
			...
			{
				return I18N['time-over'] + ((deviationTime / 1000) - _gameSettingNum)
					.toFixed(2) + 's';
			}
			SubmitResults();
		}
	...
	function cookie(name, value, time)
	{
		...
		let data = {};
		value = document.cookie.replace(/\s/g, "")
			.split(";");
	...
	{
		$("#username")
			.val(cookie("username") ? cookie("username") : "");
		$("#message")
			.val(cookie("message") ? cookie("message") : "");
		if (cookie("title"))
		{
			$('title')
				.text(cookie('title'));
	...
	w.show_setting = function()
	{
		...
		$('#setting')
			.css('display', 'block')
		$('#sound')
			.text(soundMode === 'on' ? I18N['sound-on'] : I18N['sound-off']);
	}
	...
	const clickAfterStyle = $('<style></style>'); clickBeforeStyle.appendTo($(document.head)); clickAfterStyle.appendTo($(document.head));
